...
public class LevelLoader : MonoBehaviour
{
    ...
    public void LoadFile(TextAsset levelFile)
    {
        ...
        */
        ...
        levelIndexes = new uint[(int)LevelGridSize.y, (int)LevelGridSize.x];
    ...
    public void CreateLevel()
    {
        ...
        for (int y = 0; y < (int)LevelGridSize.y; y++)
            ...
            {
                uint tileType_raw = levelIndexes[y, x];

                uint tileType = tileType_raw;
                ...
                switch (tileType)
                {
                    case 0:
                        break;
                    case 3:
                        LoadTriangleTile(x, y, tileType_raw);
                        break;
                    default:
                        LoadDefaultTile(x, y, tileType_raw);
                        break;
                }
            }
    ...
    void LoadDefaultTile(int xIndex, int yIndex, uint tileType)
    {
        // Instantiate a new tile
        GameObject newTile = new GameObject();
        newTile.name = "Tile";
        if (gameObject.name == "walls1" || gameObject.name == "walls2")
        {
    ...
    void LoadTriangleTile(int xIndex, int yIndex, uint tileType)
    {
        // Instantiate a new tile
        GameObject newTile = new GameObject();
        newTile.name = "Triangle";
        PolygonCollider2D polygonCol = newTile.AddComponent<PolygonCollider2D>();
        // Set the size of the polygon collider to the correct unity-unit tilesize.
        Vector2[] points = new Vector2[3];

        points[0] = new Vector2(-TileUnitySize.x / 2, TileUnitySize.y / 2);
        points[1] = new Vector2(-TileUnitySize.x / 2, -TileUnitySize.y / 2);
        points[2] = new Vector2(TileUnitySize.x / 2, -TileUnitySize.y / 2);
    ...
    void AddBoundaries(float levelWidth, float levelHeight)
    {
        ...
        _boundaries.layer = LayerMask.NameToLayer("Ignore Raycast");
    ...
    public void CreateOverlay()
    {
        ...
        for (int x = 0; x < LevelGridSize.x; x++)
            for (int y = 0; y < LevelGridSize.y; y++)
            {
                GameObject tile = levelTiles[y, x];
                Block block = null;
                if (tile != null)
                    block = tile.GetComponent<Block>();

                overlay.SetPixels((int)((LevelGridSize.x - 1) * TilePixelSize.x) - (int)(x * TilePixelSize.x), (int)(y * TilePixelSize.y), (int)TilePixelSize.x, (int)TilePixelSize.y, GetBlockPixels(block));
            }
    ...
    /// <returns></returns>
    Color[] GetBlockPixels(Block block)
    {
        int tileType = 0;

        if (block != null)
            tileType = (int)block.tileType;
        ...
        else
        {
            ...
            int yIndex = (TiledLevel.tilesets[0].columns - 1) - ((tileType - 1) / TiledLevel.tilesets[0].columns);

            Color[] answer = new Color[(int)TilePixelSize.x * (int)TilePixelSize.y];

            int index = 0;
            for (int y = (int)TilePixelSize.y - 1; y >= 0; --y)
                for (int x = (int)TilePixelSize.x - 1; x >= 0; --x)
                {
                    int xCoord = (int)TilePixelSize.x * xIndex + x;
                    if (block.X_Flipped)
                        xCoord = (int)TilePixelSize.x * xIndex + ((int)TilePixelSize.x - 1 - x);

                    int yCoord = (int)TilePixelSize.y * yIndex + y;
                    if (block.Y_Flipped)
                        yCoord = (int)TilePixelSize.y * yIndex + ((int)TilePixelSize.y - 1 - y);

                    answer[index] = Tileset.GetPixel(xCoord, yCoord);
                    index++;
                }
